...
function File({ walletState }) {
  ...
  useEffect(() => {
    if (walletState.address) {
      const proc = () => {
        ApiUtils.getCertifications(
          "data_fc_([A-Za-z0-9]*)_" + walletState.address,
          setCertifications
        );
      };
      proc();
  ...
  const certifyFile = async () => {
    if (acceptedFiles.length === 1 && hash && reference && uuid) {
      ...
      const tx = await WavesUtils.CertifyFile(
        reference,
        hash,
        uuid,
        timestamp,
      ...
      <div className={styles.container}>
        ...
        <div className={styles.hasharea}>
          ...
          <div className={styles.hash}>
            ...
            >
              File hash{" "}
              <div className={styles.inputTitleSm}>
                (No file is sent or stored online unless you choose IPFS option)
              </div>
            </div>
            <Input
              className={styles.inputValue}
