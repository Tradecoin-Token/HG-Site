...
function Create({ walletState }) {
  ...
  const mergeImageFileList = (oldList, newFiles) => {
    let newList = [...oldList];
    newFiles.forEach((item) => {
      if (newList.find((x) => item.path === x.path) === undefined) {
        newList.push(item);
      }
    });
    return newList;
  ...
  const onDrop = useCallback(
    (acceptedFiles) => {
      const newImageList = mergeImageFileList(imageFileList, acceptedFiles);
      if (newImageList.length > 12) {
        AlertUtils.SystemAlert("You can upload up to 12 images!");
        return;
      }
  ...
  const startAuction = async () => {
    const createAuction = async () => {
      ...
      if (imageFileList.length === 0) {
        AlertUtils.SystemAlert("Select at least one image");
        return false;
      }
      ...
      if (tx) {
        setUploading(true);
        for (let idx = 0; idx < imageFileList.length; ++idx) {
          const file = imageFileList[idx];
  ...
  return (
    <div className={styles.create}>
      <div className={styles.container}>
        ...
        <ImageList imageFileList={imageFileList} handleDelete={handleDelete} />
