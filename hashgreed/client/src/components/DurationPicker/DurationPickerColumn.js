...
DurationPickerColumn.propTypes = {
  ...
  unit: PropTypes.oneOf(["days", "hours", "mins"]).isRequired,
...
function DurationPickerColumn(props) {
  ...
  const { onChange, isSmallScreen, unit, maxDays, cellHeight } = props;
  const [columnIsFocused, setColumnIsFocused] = useState(false);
  const columnIsFocusedRef = useRef(false);
  const numCells = unit === "days" ? maxDays + 1 : unit === "hours" ? 24 : 60;
  ...
  return (
    <div
      ...
      ref={containerRef}
      role="slider"
      aria-valuemax={unit === "days" ? maxDays : unit === "hours" ? 24 : 60}
      aria-valuemin={0}
